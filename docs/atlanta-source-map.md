...
| ❌ KILLED 2026-07-17 (fall probe: ZERO music events in a full Aug–Dec window; Bailey PC absent from every location name; arts/music sites carry no machine feed — BACKLOG) — **KSU Bailey Performance Center (Localist)** | JSON API + iCal | `https://calendar.kennesaw.edu/api/2/events` | University recitals in Kennesaw (Cobb). Public read, no auth. Filter by music dept id. |
| ✅ WIRED 2026-07-17 (`gsu-localist`, `src/feeds/gsu.ts`; server-side `type[]=52620491419918` — GSU's own "Music Concerts" category, NOT `keyword`) — **GSU School of Music (Localist)** | JSON API + iCal | `https://calendar.gsu.edu/api/2/events` | Kopleff recitals + Rialto headliners (Béla Fleck, Take 6, Joshua Redman). Per-event iCal export confirmed. |
